...
def Sticker_button(win,color_key):
    ...
    button_background = pygame.draw.rect(win,color[color_key], (10, HEIGHT - 45, 110, 30), 5)
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if (event.type == pygame.MOUSEBUTTONDOWN) and pointing_2:
                if event.button == 1:
                    obj = Sticker(win)
                    if obj.sticker_selected_no != None :
                        text.recv_snd_msg.append([""] + obj.sticker_selected_no )
                    ...
                    if event.key == pygame.K_RETURN:
                        try:
                            if text.being_written_msg[-1][-1]:
                                text.recv_snd_msg.append([""] + text.being_written_msg[-1])
                                text.recv_snd_msg = n.send((text.recv_snd_msg, start_screen_obj.Name))[0]
        ...
        ##draw captions
        pygame.display.set_caption("pygame - " + start_screen_obj.Name.casefold())
